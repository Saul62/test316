...
DATA_PATH = BASE_DIR / "ards整理后数据12新.xls"
TARGET_COLUMN = "ards"
...
DISPLAY_NAMES = {
    ...
    "crrt_flag": "CRRT",
    "pneumonia": "Pneumonia",
    "liquid_balance_value": "Liquid Balance",
    "glucose": "Glucose",
    "albumin": "Albumin",
...
def build_feature_specs(df: pd.DataFrame) -> dict[str, dict[str, Any]]:
    specs: dict[str, dict[str, Any]] = {}
    for column in df.columns:
        if column == TARGET_COLUMN:
            continue
        series = df[column]
        label = DISPLAY_NAMES.get(column, column.replace("_", " ").title())
        if pd.api.types.is_numeric_dtype(series):
            specs[column] = {
                ...
                "step": 0.1 if (series.max() - series.min()) <= 100 else 1.0,
                "q1": float(series.quantile(0.25)),
                "q3": float(series.quantile(0.75)),
            }
        else:
            options = sorted(series.dropna().astype(str).unique().tolist())
            default = "No" if "No" in options else options[0]
            specs[column] = {
                "type": "categorical",
                "label": label,
                "options": options,
...
def render_header(artifact: dict[str, Any], dataset: pd.DataFrame) -> None:
    st.markdown(
        f"""
        <div class="hero-card">
...
def render_sidebar(artifact: dict[str, Any], dataset: pd.DataFrame) -> None:
...
def build_input_form(feature_specs: dict[str, dict[str, Any]]) -> tuple[bool, dict[str, Any]]:
    user_inputs: dict[str, Any] = {}

    st.markdown("### Patient Inputs")
    st.caption("All labels are in English. Defaults are pulled from the median or most common values in the local dataset.")
...
def build_shap_explainer() -> Any:
    ...
    background_df = dataset[feature_columns].sample(
...
def main() -> None:
    ...
    feature_columns = [col for col in dataset.columns if col != TARGET_COLUMN]
    feature_specs = build_feature_specs(dataset[feature_columns + [TARGET_COLUMN]])

    render_header(artifact, dataset)
    render_sidebar(artifact, dataset)

    submitted, user_inputs = build_input_form(feature_specs)
